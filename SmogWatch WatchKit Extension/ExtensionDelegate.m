...
- (void)applicationDidFinishLaunching {
    [[[KrakowPiosDataLoader alloc] init] fetchDataWithCompletionHandler:^{
        [self reloadActiveComplications];
        [self scheduleBackgroundRefreshIn:3600];
    }];
}
...
- (void)reloadActiveComplications {
    id server = [CLKComplicationServer sharedInstance];
    NSArray *complications = [server activeComplications];
...
- (void)scheduleBackgroundRefreshIn:(NSTimeInterval)seconds {
    [[WKExtension sharedExtension]
     scheduleBackgroundRefreshWithPreferredDate:[[NSDate date] dateByAddingTimeInterval:seconds]
     userInfo:nil
